...
export const SessionDetail: React.FC<SessionDetailProps> = ({ session, onBack }) => {
  ...
  const exportCSV = () => {
    const csv = messages
      .map((m) => `${m.timestamp},${m.sender},${JSON.stringify(m.text)}`)
      .join('\n');
  ...
  const exportJSON = () => {
    const blob = new Blob([JSON.stringify({ session, messages }, null, 2)], {
            ...
            <div className="space-y-4">
              {messages.map((message) => (
                <div key={message.id} className="flex">
                  ...
                  <div
                    className={`flex-1 ${message.sender === 'user' ? 'pl-4 border-l-4 border-blue-500' : 'pl-4 border-l-4 border-green-500'}`}
                  >
