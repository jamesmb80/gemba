...
import { getChatSessions, createChatSession, getChatMessages, addChatMessage, sendMessageToAI, Machine, ChatSession, ChatMessage } from '../lib/api';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ machine, onBack }) => {
  ...
  const handleRecord = () => {
    ...
    setTimeout(() => {
      ...
      setInputValue('The machine is making a loud grinding noise when starting up and displaying error code E-394.');
    }, 3000);
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto">
      ...
      <div className="bg-white p-3 rounded-t-lg shadow-md border-b border-gray-200 flex justify-between items-center">
        <button onClick={onBack} className="p-1 rounded-full hover:bg-gray-100" aria-label="Go back">
        ...
        <button onClick={() => setAudioEnabled(!audioEnabled)} className="p-1 rounded-full hover:bg-gray-100" aria-label={audioEnabled ? 'Disable audio' : 'Enable audio'}>
        ...
        {messages.length === 0 && !isProcessing ? (
          ...
          messages.map((message) => (
            <div key={message.id} className={`mb-4 flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-lg p-3 ${message.sender === 'user' ? 'bg-blue-600 text-white' : message.sender === 'ai' ? 'bg-white text-gray-800 border border-gray-200 shadow-sm' : 'bg-gray-200 text-gray-800'}`}>
                <div className="flex items-start">
                  {message.sender === 'ai' && renderConfidenceIndicator(message.confidence || '')}
                  <p className={message.sender === 'ai' ? 'ml-2' : ''}>{message.text}</p>
                </div>
                <div className={`text-xs mt-1 ${message.sender === 'user' ? 'text-blue-200' : 'text-gray-500'}`}>{new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
              </div>
            </div>
          ))
        )}
      ...
      <div className="bg-white p-3 border-t border-gray-200 rounded-b-lg shadow-md">
        <form onSubmit={handleSend} className="flex items-center">
          <button type="button" onClick={handleRecord} className={`p-3 rounded-full mr-2 ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`} aria-label="Voice input">
            <MicIcon size={20} />
          </button>
          <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder={isRecording ? 'Listening...' : 'Type your message...'} className="flex-1 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" disabled={isRecording} />
          <button type="submit" className="p-3 bg-blue-600 text-white rounded-full ml-2 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed" disabled={!inputValue.trim() || isProcessing} aria-label="Send message">
            <SendIcon size={20} />
          </button>
        </form>
      </div>
